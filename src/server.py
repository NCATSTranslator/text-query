...
mcp = FastMCP("microbiome-query")

PLOVER_ENDPOINT = "https://multiomics.test.transltr.io/mbkp/query"
META_KG_ENDPOINT = "https://multiomics.test.transltr.io/mbkp/meta_knowledge_graph"
...
async def explore_microbiome_disease_path(
    ...
    microbes1 = set()
    kg1 = result1.get("message", {}).get("knowledge_graph", {})
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "test":
        ...
        async def test():
            print("\n=== Testing MicrobiomeKG MCP Server ===\n")
            
            print("1. Getting meta knowledge graph...")
            meta_kg = await get_meta_knowledge_graph()
            print(meta_kg[:500] + "...\n")
            
            print("2. Finding microbes for NAFLD (MONDO:0013209)...")
            nafld_microbes = await find_microbiome_for_disease(
                disease_id="MONDO:0013209",
                disease_name="NAFLD"
            )
            print(nafld_microbes[:500] + "...\n")
            
            print("3. Exploring helminthiasis -> NAFLD path...")
            path = await explore_microbiome_disease_path(
                start_id="MONDO:0004664",
                end_id="MONDO:0013209",
                start_name="helminthiasis",
                end_name="NAFLD"
            )
            print(path[:500] + "...\n")
